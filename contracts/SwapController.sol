 ...
 */
contract SwapController is AccessControl, ReentrancyGuard {
     ...
     */
    function getAmountsOut(uint256 _inputAmount, address tokenIn, address tokenOut) external view returns (uint256, uint256, uint256) {
        return _getAmountsOut(_inputAmount, tokenIn, tokenOut);
    }

    function _getCoinIndexes(
        address tokenIn,
        address tokenOut
    ) internal view returns (uint8, uint8, AStructs.Coin memory, AStructs.Coin memory) {
     ...
     * @return Output amount, input amount, and direction of momentum (premium).
     */
    function _getAmountsOut(uint256 _inputAmount, address tokenIn, address tokenOut) internal view returns (uint256, uint256, uint256) {
        require(tokenIn != tokenOut, 'Input and output coins are the same');

        // Data structures to store coin details.
        (uint8 inputCoinIndex, uint8 outputCoinIndex, AStructs.Coin memory coinIn, AStructs.Coin memory coinOut) = _getCoinIndexes(
            tokenIn,
            tokenOut
        );
        ...
        if (tokenIn == address(VAULT)) {
            ...
            require(
                IERC20(coinList[outputCoinIndex + 1].token).balanceOf(address(VAULT)) + 10 ** coinList[outputCoinIndex + 1].decimals >
                    outputAmount,
        ...
        uint256 outputAmountInCoin = _calculateOutputAmount(oldBalancesInVUND, _inputAmount, coinIn, inputCoinIndex, outputCoinIndex);
        ...
        uint256 premium = (_calculateDifference(oldBalancesInVUND[inputCoinIndex], oldBalancesInVUND[outputCoinIndex]) >
    ...
    function _calculateOutputAmount(
        ...
        for (uint i = 0; i < oldBalancesInVUND.length; i++) {
    ...
    function SwapExecution(
        ...
        (uint256 outputAmount, uint256 inputAmount, uint256 comissionPct) = _getAmountsOut(_inputAmount, _tokenIn, _tokenOut);
        ...
        if (_slippageTolerance > 0) {
            ...
            require(
                outputAmountInVUND >= inputAmountInVUND - ((_slippageTolerance + comissionPct) * inputAmountInVUND) / pct_denom,
     ...
     */
    function ramp_A(uint256 _future_A, uint256 _future_time) external onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     */
    function stop_ramp_A() external onlyRole(DEFAULT_ADMIN_ROLE) {
