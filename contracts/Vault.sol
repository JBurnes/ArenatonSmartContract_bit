 ...
 */
contract Vault is AccessControl, ReentrancyGuard, ERC20 {
     ...
     */
    constructor(address[] memory _coins, address _ATON) ERC20('Vault Unity Dollar', 'VUND') {
     ...
     */
    function toggleCoinActiveStatus(address _coinAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     */
    function addAuthorizedAddress(address authorizedAddress, string memory role) public onlyRole(DEFAULT_ADMIN_ROLE) {
        grantRole(keccak256(abi.encodePacked(role)), authorizedAddress);
    }
     ...
     */
    function removeAuthorizedAddress(address authorizedAddress, string memory role) public onlyRole(DEFAULT_ADMIN_ROLE) {
        revokeRole(keccak256(abi.encodePacked(role)), authorizedAddress);
    }
    ...
    function retrieveCoin(
        ...
        if (_token == address(this)) {
            ...
            require(token.allowance(_player, address(this)) >= _amountCoin);
    ...
    function addEvent(
        ...
        require(_startDate > block.timestamp && !events[_eventIdBytes].active && !_ifEventExists(_eventIdBytes), 'Invalid event setup');
    ...
    function _getEventDTO(bytes8 _eventIdBytes) internal view returns (AStructs.EventDTO memory) {
        ...
        return (
            AStructs.EventDTO(
                ...
                events[_eventIdBytes].canceledVUND[0], // Total VUND staked for Team A
                events[_eventIdBytes].canceledVUND[1], // Total VUND staked for Team B
    ...
    function addStake(
        ...
        currentEvent.totalVUND[_team] += amountVUND;
        currentEvent.totalATON[_team] += _amountATON;
    ...
    function closeEvent(
        ...
        // Transfer Cancelled stakes fees to other Team
        if (_winner == 1) {
            if (events[_eventIdBytes].canceledVUND[1] > 0) {
                events[_eventIdBytes].totalVUND[1] -= events[_eventIdBytes].canceledVUND[1];
                events[_eventIdBytes].totalVUND[0] += events[_eventIdBytes].canceledVUND[1];
            }
        } else if (_winner == 0) {
            if (events[_eventIdBytes].canceledVUND[0] > 0) {
                events[_eventIdBytes].totalVUND[0] -= events[_eventIdBytes].canceledVUND[0];
                events[_eventIdBytes].totalVUND[1] += events[_eventIdBytes].canceledVUND[0];
            }
        }
    ...
    function getEventStakedVUND(bytes8 _eventIdBytes, uint8 _context) external view returns (uint256) {
        if (_context == AStructs.WholeRawVUND) {
            return events[_eventIdBytes].totalVUND[AStructs.TEAM_A] + events[_eventIdBytes].totalVUND[AStructs.TEAM_B];
        } else if (_context == AStructs.TeamARawVUND) {
            return events[_eventIdBytes].totalVUND[AStructs.TEAM_A];
        } else if (_context == AStructs.TeamBRawVUND) {
            return events[_eventIdBytes].totalVUND[AStructs.TEAM_B];
        } else {
            ...
            if (_context == AStructs.WholeEffectiveVUND) {
                ...
                for (uint i = 0; i < events[_eventIdBytes].totalVUND.length; i++) {
                    effectiveEventVUND += (events[_eventIdBytes].totalVUND[i] +
                        (events[_eventIdBytes].totalATON[i] * factorATON) /
                        pct_denom);
                }
                return effectiveEventVUND;
            } else if (_context == AStructs.TeamAEffectiveVUND) {
                return
                    events[_eventIdBytes].totalVUND[AStructs.TEAM_A] +
                    (events[_eventIdBytes].totalATON[AStructs.TEAM_A] * factorATON) /
                    pct_denom;
            } else if (_context == AStructs.TeamBEffectiveVUND) {
                return
                    events[_eventIdBytes].totalVUND[AStructs.TEAM_B] +
                    (events[_eventIdBytes].totalATON[AStructs.TEAM_B] * factorATON) /
                    pct_denom;
            }
        }
    ...
    function _getPlayerStake(bytes8 _eventIdBytes, address _player) internal view returns (AStructs.StakeDTO memory) {
        ...
        return AStructs.StakeDTO(stake.amountVUND, stake.amountATON, stake.team, effectivePlayerStake);
    }
     ...
     */
    function setPlayerFinalizedEvent(bytes8 _eventIdBytes, address _player) external onlyRole(ARENATON_ROLE) returns (bool) {
        ...
        if (events[_eventIdBytes].stakes[_player].amountVUND > 0) {
            ...
            } else if (events[_eventIdBytes].winner == -2) {
                players[_player].level += 2;
            } else {
                players[_player].level += 1;
            }

            players[_player].eventCounter[0] += 1;
            players[_player].eventCounter[events[_eventIdBytes].sport] += 1;
        }
        return true;
    }
     ...
     */
    function getEventCounter(address _player, uint8 _sport) external view returns (uint256) {
        return players[_player].eventCounter[_sport];
    }
    ...
    function _addEarningsToPlayer(
        ...
        if (_amountVUND > 0) {
            if (AStructs.VaultFee == _category) {
    ...
    /**
     * @notice Allows a player to cancel their stake for an event, returning a portion of their staked VUND after deducting a cancellation fee.
     * @dev Assumes the existence of several mappings and properties not shown in this code snippet. Always ensure that the cancelationPctCost is a valid percentage.
     * @param _eventIdBytes The ID of the event in bytes format.
     * @param _player The address of the player whose stake is being canceled.
     * @param _cancelationPctCost The percentage of VUND to be taken as a cancellation fee.
     */
    function cancelPlayerStake(
        bytes8 _eventIdBytes,
        address _player,
        uint256 _cancelationPctCost
    ) external onlyRole(ARENATON_ROLE) nonReentrant {
        // Ensuring the event is still ongoing and not started yet
        require(events[_eventIdBytes].startDate > block.timestamp, 'Event already started');

        // Fetching the player's staking details
        uint256 amountVUND = events[_eventIdBytes].stakes[_player].amountVUND;
        uint256 amountATON = events[_eventIdBytes].stakes[_player].amountATON;
        uint8 team = events[_eventIdBytes].stakes[_player].team;

        // Calculating the refundable amount after deduction of the cancellation fee
        uint256 amountVUNDtoReturn = amountVUND - (amountVUND * _cancelationPctCost) / pct_denom;

        // Protect against potential underflow
        require(events[_eventIdBytes].totalVUND[team] >= amountVUNDtoReturn, 'Underflow protection');
        require(events[_eventIdBytes].totalATON[team] >= amountATON, 'Underflow protection');

        // Locating the player's index in the event's players array to remove them
        ...
        // Ensure the player exists in the array before removing
        ...
        // Adjusting total staked amounts for the team after cancellation
        events[_eventIdBytes].totalVUND[team] -= amountVUNDtoReturn;
        events[_eventIdBytes].totalATON[team] -= amountATON;

        // Reducing the total stake count for the event
        events[_eventIdBytes].stakeCount -= 1;

        // Resetting the stake details for the player in the event
        events[_eventIdBytes].stakes[_player].amountVUND = 0;
        events[_eventIdBytes].stakes[_player].amountATON = 0;
        events[_eventIdBytes].stakes[_player].team = 0;

        // Logging the amount deducted as cancellation fee
        events[_eventIdBytes].canceledVUND[team] += (amountVUND * _cancelationPctCost) / pct_denom;

        // Removing the event from the player's active events list
        _removePlayerActiveEvent(_eventIdBytes, _player);

        // Transferring the refundable VUND back to the player
        _transfer(address(this), _player, amountVUNDtoReturn);
    }
     ...
     * @param _comissionPct Commission percentage for the swap.
    ...
    function swap(
        ...
        (uint256 comissionVUND, ) = Tools.convertCoinToVUND((_amountOut * _comissionPct) / pct_denom, coinOut);
        _accumulateCommission(comissionVUND);
        ...
        _amountOut = _amountOut - (_amountOut * _comissionPct) / pct_denom;
    ...
    function _mintToRebalance() internal returns (bool) {
        ...
        for (uint i = 1; i < coinList.length; i++) {
            ERC20 token = ERC20(coinList[i].token);
    ...
    function _distributeCommission(address player) internal {
        ...
        if (unclaimedCommission > 0) {
            ...
            emit EventsLib.Earnings('', player, '', player, unclaimedCommission, 0, AStructs.Comission);
        }
    }
    ...
    function donateVUND(address _player, uint256 _amount) external onlyRole(ARENATON_ROLE) {
        _transfer(_player, address(this), _amount);
        _accumulateCommission(_amount);

        emit EventsLib.Earnings('', address(this), '', address(this), _amount, 0, AStructs.Comission);
    }
}
